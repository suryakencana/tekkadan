...
function adjustPositionTo(target, callback, offset) {
  var actualOffset = 5;
  if(typeof offset != "undefined" && offset != null) actualOffset = offset;
  $('html, body').animate({
    scrollTop: target.offset().top - actualOffset + 'px'
  }, 500, callback);
}
...
// TODO consider change method to support >= 2 day flights
var getDuration = function (hour, min, TZ, toHour, toMin, toTZ) {
  var C = 10000; // C to avoid negative number;
  var from  = hour*60   + min*1   - TZ    + C,
      to    = toHour*60 + toMin*1 - toTZ  + C;
  ...
  var diff              = to - from,
      durationInHour    = Math.floor(diff/60),
      durationInMin     = diff%60,
      durationInString  = durationInHour + 'j ' + durationInMin + 'm';
  return {inString : durationInString, nextDay: nextDay, inHour: durationInHour, inMin: durationInMin} ;
};
...
function getPriceIDR(price) {
  return {main: getMainPrice(price), tail:getTailPrice(price)};
}

function getMainPrice(price) {
  if (price < 0)
  ...
  if (price < 1000)
    return price;
  var headPrice = Math.floor(price/1000);
  var tailPrice = ('000' + price % 1000).slice(-3);

  return headPrice + '.' + tailPrice;
}

function getTailPrice(price) {
  if (price < 0)
    return getTailPrice(-price);
  if (price < 1000)
    return price;
  return '.' + ('000' + (price%1000)).slice(-3);
}
...
$(function () {
  ...
  $('#loginPopup').click(createLoginDialog);
  $('#registerPopup').click(createRegisterDialog);
...
function createLoginDialog() {
    closeAllDialogBox();
    $('#LoginRegister').qtip('hide');

    var loginDialog = $('<div class=""><div class="loadingMessage">Sedang melakukan koneksi ke server...</div></div>')
                    .hide()
                    .load($('#loginPopup').attr('href'), function(response, status, xhr) {
                      if ( xhr.status === 200 ) {
                        initSimpleLogin();
                        $(this).dialog('open');
                        $('#tvAlert').dialog('close');
                      }
                    })
                    .dialog({
                      autoOpen  : false,
    ...
    dialogList.push(loginDialog);

    $.tvAlert.addAlertMsg({
          msg: "Sedang melakukan koneksi ke server...",
          showCSImage: true,
          includeHeader: true,
          headerImageClass: "tvGreyLogo",
          showCloseBtn: true,
...
function createRegisterDialog() {
     ...
     var registerDialog = $('<div class=""><div class="loadingMessage">Sedang melakukan koneksi ke server...</div></div>')
                        .hide()
                        .load($('#registerPopup').attr('href'), function(response, status, xhr) {
                          if ( xhr.status === 200 ) {
                            setTimeout(initSimpleRegister, 300);
                            $(this).dialog('open');
                            $('#tvAlert').dialog('close');
                          }
                        })
     ...
     dialogList.push(registerDialog);
...
function initSimpleLogin() {
  $('#simpleRegister').click(function() {return createRegisterDialog();});
  $('#simpleLoginBtn').click(submitLoginRequest);
  $('#simpleEmail').focus();
  $('#simplePassword').keypress(function(e) {
    var code = (e.keyCode ? e.keyCode : e.which);
    if ( code === 13 ) submitLoginRequest();
  });

  $('#fbButton').click( function (e) {
    initFbLogin('/logintofb', '');
    return false;
  });
  $('.tvCloseGreyIcon').click(closeAllDialogBox);
}

function initSimpleRegister() {
  var airportQtip = {position : {
                       my: 'top center',
                       at: 'bottom center'},
                     style: {
                       tip: true,
  ...
  $('#airport').combobox({qtip: airportQtip, autocomplete: { appendTo: "#comboBoxCntr" }});
  $('#simpleLogin').click(function() {return createLoginDialog();})
  $('#simpleEmail').focus();
  $('#simpleSignUpBtn').click(submitRegisterRequest);
  $('#PP').click( function (e) {
    openPopupWindow('/privacypolicy', 'Privacy Policy', 960, 600);
    return false;
  });
  $('#TnC').click( function (e) {
    openPopupWindow('/termsandconditions', 'Terms and Conditions', 960, 600);
    return false;
  });
  $('#fbButton').click( function (e) {
    initFbLogin('/logintofb', '');
...
function initFbLogin(url, urlToRedirect) {
    var win = openPopupWindow(url, 'Facebook Login', 450, 300);

    var pollTimer = window.setInterval(function() {
      var mustClose = false;
      if ( win.isConnectedToFb === true ) mustClose = true;
      if ( mustClose ) {
        win.isConnectedToFb = false;
        win.close();
        window.clearInterval(pollTimer);
        if ( urlToRedirect === '' ) location.reload();
        else window.location = urlToRedirect;
      }
    }, 500);
}
...
function submitLoginRequest() {
  var email 		  = $('#simpleEmail').val(),
      password	  = $('#simplePassword').val(),
      rememberMe  = $('#simpleRememberMe').attr('checked') ? "true" : "false";
  ...
  $.ajax({
    ...
    beforeSend  : function() {
      waitingProcess('#loginBtn', 'Mohon tunggu...');
    },
    complete		  : function(xhr) {
      releaseWaitingProcess('#loginBtn', 'Login');
      if ( xhr.status !== 200 ) {
        $('#loginErrorMessage').html("Terjadi Kesalahan Internal. Silakan coba lagi.");
        return;
      }

      var retData = JSON.parse(xhr.responseText),
          status  = retData.metadata;

      if ( status === "validationError" ) {
        var errorList = retData.data.data;
        var errorMsg  = "";
        for ( var key in errorList ) {
          if ( typeof errorList[key][':e'] !== 'undefined' ) {
            errorMsg = errorList[key][':e'];
            break;
          }
        }
        ...
        $('#loginErrorMessage').html(retData.data);
      }
      else if ( status === "unverified" ) {
        var msg = "Silakan cek email Anda untuk memverifikasi akun atau kirim ulang <a href='/resendverification'>kode verifikasi</a> jika belum menerimanya";
...
function submitRegisterRequest() {
  ...
  $.ajax({
    ...
    beforeSend  : function() {
      waitingProcess('#simpleSignUpBtn', 'Sedang mendaftarkan...');
    },
    complete     : function(xhr) {
      releaseWaitingProcess('#simpleSignUpBtn', 'Daftar');
      if ( xhr.status !== 200 ) {
        $('#registerErrorMessage').html('Terjadi Kesalahan Internal. Silakan coba lagi.');
        return;
      }
      ...
      if ( status === "validationError" ) {
        ...
        $('#registerErrorMessage').html(errorMsg);
      }
      else if ( status === "error" )
        $('#registerErrorMessage').html(retData.data);
      else if ( status === "isVerified" )
        $('#registerErrorMessage').html("Akun ini sudah terdaftar. Silakan <a href='/login'>login</a>.");
      else if ( status === "notVerified" )
        $('#registerErrorMessage').html("Silakan konfirmasi email Anda terlebih dahulu atau <a href=\"/resendverification\">klik di sini</a> untuk mengirim ulang kode verifikasi");
      else {
        closeAllDialogBox();
        $.tvError.addErrorMsg({
              className : "tvSuccess float",
              msg       :  retData.data,
              duration  : 10000
        });
      }
